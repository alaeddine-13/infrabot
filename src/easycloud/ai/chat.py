...
class ChatSession:
    ...
    def start_chat(self, component_name: Optional[str] = None):
        ...
        if not context:
            if component_name:
                rprint(f"[bold red]No Terraform configuration found for component '{component_name}'[/bold red]")
                return
            else:
                rprint("[bold yellow]No Terraform configurations found in the project[/bold yellow]")
                return

        # Initialize conversation with system message and context
        self.conversation_history = [
            {"role": "system", "content": self.system_prompt},
            {"role": "system", "content": f"Here is the Terraform configuration context:\n{context}"}
        ]

        rprint("[bold green]Chat session started. Type 'exit' or 'quit' to end the session.[/bold green]")
